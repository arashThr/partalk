...
class TelegramBot extends EventEmitter {
    ...
    getProcessUpdate() {
        return this.processUpdate.bind(this);
    }
    ...
    _request(_path, options = {}) {
        ...
        if (options.form) {
            TelegramBot._fixReplyMarkup(options.form);
        }
        if (options.qs) {
            TelegramBot._fixReplyMarkup(options.qs);
        }
        ...
        return request(options)
            .then(resp => {
                ...
                logger.error('Error in setting up webhook for ' + this.token, err.message);
            });
    }
     ...
     *
     * @param  {String} fileId  File identifier to get info about
     * @return {BPromise} promise Promise which will have *fileURI* in resolve callback
     * @see https://core.telegram.org/bots/api#getfile
     */
    getFileLink(fileId) {
     ...
     * @return {BPromise} promise Promise, which will have *filePath* of downloaded file in resolve callback
     */
